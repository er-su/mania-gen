...
class OsuGen(keras.Model):
    ...
    def __init__(self, hyperparams, num_keys=4):
        ...
        # Generating onsets
        self.onset_GRU = layers.Bidirectional(layers.GRU(self.onset_gru_size, return_sequences=True), merge_mode="ave")
    ...
    def call(self, mel, beat_frac, beat_num):
...
model.compile()
model.build(input_shape={"mel": (3, None, 5),
                         "beat_frac": (None, 1),
                         "beat_num": (None, 1)})
model.summary()